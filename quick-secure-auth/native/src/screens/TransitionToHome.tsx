...
const DOT_SIZE = 24;
const S = 5; // apparent scale at launch: 24 × 5 = 120px
...
export function TransitionToHome({
  ...
  // Bezier start, control + end points — all in container-local coords, set in startFlight
  ...
  const t             = useSharedValue(0);   // drives the deterministic bezier arc
  const recoilOffset  = useSharedValue(0);   // liftoff recoil on Y
  const avatarScale   = useSharedValue(S);
  const smearX        = useSharedValue(1);   // squash-and-stretch at peak velocity
  ...
  const startFlight = useCallback((px: number, py: number, pw: number, ph: number) => {
    ...
    // All measureInWindow results are window coords. Subtract screenOffsetY
    // (the Y of screenArea in the window) to convert to local container coords.
    ...
    // Commit initial position (overlay still fully opaque — no flash)
    startTxSV.value = sTx;
    startTySV.value = sTy;
    ...
    // Scale: fast-committed launch → graceful arrival → spring snap
  ...
  return (
    <View style={StyleSheet.absoluteFill}>
      {/* Home screen fully rendered — measureInWindow reliable at opacity:1 */}
      <HomeScreen
        userName={userName}
      ...
      {/* Solid overlay covers home, fades as avatar travels */}
      <Animated.View
        style={[StyleSheet.absoluteFill, styles.overlay, overlayStyle]}
        pointerEvents="none"
      />

      {/* Flying dot — above everything */}
...
const styles = StyleSheet.create({
  ...
  // fontSize 8 × scale 5 = 40px apparent at launch — matches auth screen
  initials: {
    fontSize: 8,
