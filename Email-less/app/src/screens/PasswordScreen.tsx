...
import { ArrowLeft, Eye, EyeOff, Copy, Check, X } from 'lucide-react'
...
export function PasswordScreen({ onBack, onForgot, onLogin, username = 'johndoe', company = 'Woolworths', orgId = 'ORG-XXXX-1234', demoError = false, demoDisabled = false }: PasswordScreenProps) {
        ...
        <div className="flex flex-col w-[370px]" style={{ gap: 24 }}>
          ...
          <div className="bg-surface rounded-[8px] flex flex-col w-full" style={{ paddingTop: 24, paddingBottom: 40, paddingLeft: 16, paddingRight: 16, gap: 24, boxShadow: '0px 2.4px 8px rgba(0,0,0,0.06), 0px 14px 28px rgba(0,0,0,0.16)' }}>
            <div className="flex items-center" style={{ gap: 8 }}>
              ...
              <span className="text-surface font-bold" style={{ fontSize: 24, lineHeight: '32px', letterSpacing: '-0.5px' }}>Log in</span>
            </div>
            <div className="flex flex-col w-full" style={{ gap: 24 }}>
              <p className="text-surface-weaker" style={{ fontSize: 14, lineHeight: '20px', margin: 0 }}>
                Log in to{' '}<span className="text-surface font-medium">{company}</span>{' '}as{' '}<span className="text-surface font-medium">{username}</span>
              </p>
              <div className="flex flex-col w-full" style={{ gap: 4 }}>
                <label htmlFor="password" className="font-medium" style={{ fontSize: 14, lineHeight: '20px', color: demoError ? 'var(--text-color-negative)' : 'var(--text-color-surface)' }}>Password</label>
                <div className="relative w-full">
                  <input id="password" type={showPassword ? 'text' : 'password'} disabled={demoDisabled} placeholder="" className={inputCls} style={{ height: 40, paddingLeft: 12, paddingRight: 40, fontSize: 14, lineHeight: '20px' }} />
                  <button type="button" tabIndex={-1} disabled={demoDisabled} onClick={() => setShowPassword(v => !v)} className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center justify-center w-6 h-6 bg-transparent border-0 cursor-pointer" style={{ color: demoDisabled ? 'var(--text-color-surface-disabled)' : 'var(--text-color-surface-weaker)' }} aria-label={showPassword ? 'Hide password' : 'Show password'}>
                    {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                  </button>
                </div>
                {demoError && <p className="text-negative font-normal" style={{ fontSize: 12, lineHeight: '16px', margin: 0 }}>Incorrect password. Please try again.</p>}
              </div>
              <div className="flex flex-col w-full" style={{ gap: 4 }}>
              ...
              <button disabled={demoDisabled} onClick={onLogin} className={primaryCls} style={{ height: 40, fontSize: 14, lineHeight: '20px' }}>Log in</button>
              <button disabled={demoDisabled} onClick={onForgot} className={tertiaryCls} style={{ height: 40, fontSize: 14, lineHeight: '20px' }}>Forgot password?</button>
            </div>
          </div>
        </div>
      </main>
